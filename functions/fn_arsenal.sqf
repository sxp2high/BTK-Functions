/*
	File: fn_arsenal.sqf
	Author: Karel Moricky, sxp2high (BTK) (btk@arma3.cc)

	Description:
	Main arsenal functions
...
// Close previous dialogs, especially inventory
[] spawn {
	...
	closeDialog 0; // Close previous dialogs
	btk_keys_current = []; // Fix key handler

	waitUntil {!(isNull (uiNamespace getVariable ["BIS_fnc_arsenal_cam", objnull]))};
	
	waitUntil {!(isNull (findDisplay 602)) || (isNull (uiNamespace getVariable ["BIS_fnc_arsenal_cam", objnull]))};

	if (!(isNull (findDisplay 602))) then { _gearDisplay closeDisplay 1; };
